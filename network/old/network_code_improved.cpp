...
public:
    ...
    [[nodiscard]] virtual iterator begin(const size_t&,
                                         EDGE_TYPE) const = 0;
    [[nodiscard]] virtual iterator end(const size_t&,
                                       EDGE_TYPE) const = 0;
};
...
public:
    iterator() = delete;
    explicit iterator(const EdgeIteratorInterface*&);
    bool operator!=(iterator const& other) const;
    bool operator==(iterator const& other) const;
    ~iterator();
    
    iterator& operator++();
    iterator& operator=(EdgeIteratorInterface&&);
...
NetworkInterface<FlowType>::iterator::iterator(
        const NetworkInterface<FlowType>::EdgeIteratorInterface*& pointer):
            _pointer(pointer){}
...
typename NetworkInterface<FlowType>::iterator&
NetworkInterface<FlowType>::iterator::operator=(
        NetworkInterface<FlowType>::EdgeIteratorInterface&& other) {
    delete _pointer;
...
public:
    ...
    explicit EdgeIteratorInterface(const NetworkInterface<FlowType>&);
    
    virtual bool is_equal(const EdgeIteratorInterface& other) const = 0;
    
    virtual EdgeIteratorInterface& operator++() = 0;
    
    virtual FlowType get_free_capacity() const = 0;
...
NetworkInterface<FlowType>::EdgeIteratorInterface::EdgeIteratorInterface(
        const NetworkInterface<FlowType>& network): _network(network) {}
...
int main() {
    ...
    std::tie(answer, error_check) = solver.solve<MKMMaxFlowFinder<int>, GoldbergRTFMaxFlowFinder<int>>(cin);
    if(!error_check)
        return 1;
    cout << answer;
    return 0;
